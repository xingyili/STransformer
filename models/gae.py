...
class IGAE_encoder(nn.Module):
    ...
    def __init__(self, gae_n_enc_1, gae_n_enc_2, gae_n_enc_3, n_input):
        ...
        # 编码器残差连接
        self.enc_res_linear = GNNLayer(n_input, gae_n_enc_3) if n_input != gae_n_enc_3 else None
    ...
    def forward(self, x, adj):
        ...
        # add residual
        if self.enc_res_linear:
            identity = self.enc_res_linear(x, adj)
            z_igae += identity
...
class IGAE_decoder(nn.Module):
    ...
    def __init__(self, gae_n_dec_1, gae_n_dec_2, gae_n_dec_3, n_input):
        ...
        # 编码器残差连接
        self.dec_res_linear = nn.Linear(gae_n_dec_1, n_input) if n_input != gae_n_dec_1 else None
    ...
    def forward(self, z_igae, adj):
        ...
        # add residual
        # if self.dec_res_linear:
        #     identity = self.dec_res_linear(z_igae)
        #     z_hat += identity
